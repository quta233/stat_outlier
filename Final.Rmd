...
Fig. 8 : A regression of proportion of outliers detected by a given a method and skewness of a dataset:
...
```{r}
Scook <- lm(pcook~skewness, data=record)
gf_point(pcook ~skewness,
data = record) %>%
gf_smooth(linetype = "dashed",
color = "red") %>%
gf_lm(size = 1.5)
...
When accounting for Skewness, there are noticable increases in proportion detected for the IQR method when the skewness of dataset is larger and positive,right-skewed. 
