...
record %>%
  ...
  ggtitle("Proportion of outliers in each dataset detected by each methods")
```
...
record %>%
  ...
  ggplot(aes(x = distribution, y = proportion, color = methods)) +
  geom_boxplot() +
  facet_wrap(~methods) +
  coord_flip() +
...
Fig. X: A linear regression of the Standard Deviation Proportion on the IQR Proportion 
```{r, echo=FALSE, message=FALSE}
...
gf_lm(size = 1.5)
```
Fig X+1: A one way ANOVA Test between the IQR and Standard Deviations mean proportions
```{r, echo=FALSE}
a1 <- aov(iqvsd)
summary(a1)
```
We can see that there is significant evidence to reject the assumption that the means of proportions of the two tests are equal.

Fig. X+2: A  A linear regression of the Cook's Distance Proportion on the Chis-Square Proportion 
```{r, echo=FALSE}
...
gf_lm(size = 1.5)
```
Fig. X+3: An ANOVA test of the difference of means of sample proportions between the Chi Square and Cook's Distance tests
```{r, echo=FALSE}
a2 <-aov(csvck)
summary(a2)
```
...
Fig X+4:
```{r}
cqviq<-lm(pcsq ~ piqr, data=record)
a3 <- aov(cqviq)
summary(a3)
...
mean(record$pbonf)
```
Because of The ANOVA tests, we know that the differences in means between Cook and Chi-Square are significant as well as between Standard Deviation and IQR and Chi-Square and IQR.
